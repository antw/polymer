module Flexo::Processors
  class Sass

    TEMPLATE = Pathname.new(__FILE__).dirname + '../templates/sass_mixins.erb'

    # Given a project, the Sass processor builds a Sass stylesheet which can
    # can be included into your own Sass stylesheets, simplifying use of the
    # sprite images generated by Flexo.
    #
    # @param [Flexo::Project] project
    #   The project instance for which to generate a Sass stylesheet.
    #
    # @return [Boolean]
    #   Returns true if the stylesheet was generated and saved to the location
    #   specified by the project, or false if the project disables generation
    #   of Sass.
    #
    def self.process(project)
      return false unless project.paths.sass

      if project.paths.sass.to_s[-5..-1] == '.sass'
        project.paths.sass.dirname.mkpath
        save_to = project.paths.sass
      else
        project.paths.sass.mkpath
        save_to = project.paths.sass + '_flexo.sass'
      end

      File.open(save_to, 'w') do |file|
        file.puts ERB.new(File.read(TEMPLATE), nil, '<>').result(binding)
      end

      true
    end # self.process

    # Formats a say_status message.
    def self.format_after_message(*)
      [ 'written', 'Sass mixin', :green ]
    end

  end # Sass
end # Flexo::Processors
